var url = document.URL, title = document.title, 
    if (url === 'https://majsoul.union-game.com/0/v0.4.1.w/user_xieyi/1.txt') {
        window.loopcnt = 0; //计数
...
if (url.match(/www.tsdm.love/)) {
    if (title.match(/生日/)) {
        document.getElementById('fastpostmessage').value += '生日快樂的說…';
    }
    if (title.match(/月历下载/)) {
        document.getElementById('fastpostmessage').value += '感謝分享的說…';
    }
}
